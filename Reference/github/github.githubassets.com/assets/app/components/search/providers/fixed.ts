...
import {CaretPositionKind, isContentNode, isQualifier} from '@github-ui/blackbird-query-parsing/common'
import type {ParsedIntermediateRepresentation} from '../qbsearch-input-element'
import type {BaseNode} from '@github-ui/blackbird-parser'
...
export class FixedValuesProvider extends EventTarget implements FilterProvider {
  ...
  async handleEvent(event: QueryEvent) {
    ...
    if (!this.#parsing) {
      this.#parsing = await import('@github-ui/blackbird-query-parsing/parsing')
    }
