...
import type {QualifierNode, NodeWithChildren, ContentNode} from '@github-ui/blackbird-parser'
import {
  CaretPositionKind,
  extractRepoOrgScopes,
  hasChildren,
  isQualifier,
} from '@github-ui/blackbird-query-parsing/common'
...
export class InputProvider extends EventTarget implements SearchProvider {
  ...
  async handleEvent(event: QueryEvent) {
    ...
    let business = this.#input.getAttribute('data-current-business')

    if (!this.#parser) {
      this.#parser = await import('@github-ui/blackbird-query-parsing/parsing')
    }
    ...
    if (astNode.children) {
      ...
      business =
        (qualifierNodes.find(node => node.qualifier === 'Enterprise')?.content as ContentNode)?.value?.toString() ||
        business
      // Get owner from nwo
      if (repo && !orgNode) {
    ...
    if (state.ast) {
      ...
      if (business && business.length > 0) {
        suggestions.push({
          query: `enterprise:${business} ${trimmedQuery}`,
          scope: 'ENTERPRISE',
        })
      }
    }
