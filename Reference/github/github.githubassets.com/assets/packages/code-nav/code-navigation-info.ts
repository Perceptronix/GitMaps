...
export class CodeNavigationInfo {
  ...
  blobLines: string[]
  symbols: CodeSymbol[]
  lineIndexedSymbols: Map<number, CodeSymbol> = new Map()
  isPlain: boolean
  ...
  initSymbols(symbols: BlackbirdSymbol[]) {
    return symbols.map(outlineSymbol => {
      ...
      this.lineIndexedSymbols.set(codeSymbol.lineNumber, codeSymbol)
  ...
  getSymbolOnLine(lineNumber: number): CodeSymbol | undefined {
    return this.lineIndexedSymbols.get(lineNumber)
  }
...
export function searchInBlobIncrementally(
  ...
  const checkedLine = new Set<number>()
  const searchResults = codeRefs.reduce<Range[]>((output, ref) => {
    if (checkedLine.has(ref.ident.start.line)) {
      return output
    }

    checkedLine.add(ref.ident.start.line)
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
