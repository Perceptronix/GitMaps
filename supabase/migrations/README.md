...
- **Layout (no new migration)** — the semantic-map layout (Phase 8, D-04) writes the existing `clusters.centroid_x/y` (migration 07) and `repos.map_x/y` (migration 02) columns: a deterministic cluster-anchored projection (metric MDS on centroids + PCA scatter) that replaces UMAP, driven by `ingestion_state.layout_version` (`map_x IS NULL` ⇒ due for the incremental centroid-anchor pass). Additive columns were already reserved, so the layout is code-only.
